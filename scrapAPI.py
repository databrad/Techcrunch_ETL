...
def get_articles():
    try:
        category = request.args.get('category')  # Récupérer la catégorie à partir des paramètres d'URL
        db_connection = mysql.connector.connect(**db_config)
        cursor = db_connection.cursor(dictionary=True)

        if category:
            query = "SELECT * FROM articles WHERE category = %s"
            cursor.execute(query, (category,)) #Récupérer les articles de la catégorie en question
        else:
